// Tipos para Módulos Compostos (sem alterar tabelas)
...
export interface ModuloCompleto {
  ...
  tipo_conteudo?: string | null; // 'VIDEO', 'TEXTO', 'QUIZ', 'MISTO'
  obrigatorio: boolean;
  xp_modulo: number;
  criado_em: string;
...
export type TipoConteudoModulo = 
  | 'VIDEO'       // Apenas vídeo
  | 'TEXTO'       // Apenas texto/HTML
  | 'PDF'         // Apenas documento
  | 'QUIZ'        // Apenas avaliação
  | 'MISTO';      // Vídeo + Quiz, ou múltiplos materiais
